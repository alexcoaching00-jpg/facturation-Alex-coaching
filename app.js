...
function collectInvoice(){const client=state.clients.find(x=>x.id===document.querySelector("#invoice-client").value),items=lines.map(x=>({...x,description:x.description.trim()})).filter(x=>x.description&&Number(x.quantity)>0);if(!client){flash("Choisis d'abord un client dans la liste déroulante.");return}if(!items.length){flash("Ajoute une prestation avec une description, une quantité et un prix.");return}const date=document.querySelector("#invoice-date").value,due=document.querySelector("#invoice-due").value;if(!date||!due){flash("Renseigne la date de facture et l'échéance.");return}const t=totals(items);return {id:id(),number:nextNumber(date),clientId:client.id,clientName:client.name,date,due,items,note:document.querySelector("#invoice-note").value.trim(),...t,status:"draft",profile:{...state.profile},createdAt:new Date().toISOString()}}
...
async function emailInvoice(invoice){
  ...
  const body = encodeURIComponent(
    ...
    `Coach sportif · Wattrelos\n`+
    (state.profile.businessPhone ? `${state.profile.businessPhone}\n` : "")+
    (state.profile.businessEmail ? `${state.profile.businessEmail}` : "")
  );
...
document.querySelector("#invoice-form").addEventListener("submit",e=>{
  ...
  if(!x){
    showSend("");
    sendState("error","Vérifie : client sélectionné, au moins une prestation avec quantité et prix, et les deux dates renseignées.");
    hideSendLater(4500);
    return;
  }
  emailInvoice(x);
